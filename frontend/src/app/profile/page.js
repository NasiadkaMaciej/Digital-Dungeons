...
import { gamesApi, usersApi, likesApi } from '@/lib/api';
...
setLoadingLikedGames(true);
try {
const liked = await likesApi.getUserLikes(displayUser.userId, 100, 0);
setLikedGames(liked || []);
} catch (err) {
console.error('Failed to load liked games:', err);
} finally {
setLoadingLikedGames(false);
}
};
...
const handleUnlikeLikedGame = async (gameId) => {
try {
...
const filteredLikedGames = likedGames.filter(game =>
game.title.toLowerCase().includes(searchLikedGames.toLowerCase()) ||
(game.description && game.description.toLowerCase().includes(searchLikedGames.toLowerCase())) ||
(game.author && game.author.toLowerCase().includes(searchLikedGames.toLowerCase()))
);
...
<p className="text-foreground/60 font-mono text-sm text-center pt-5 pb-10"><Link href={'/editor'} className={'text-red-500 hover:underline hover:text-red-700'}>Create</Link> your first game.</p>
...
<div className="px-12 py-10 bg-background rounded-lg border-1 border-red-500">
<h2 className="text-3xl font-black mb-10">Liked Games</h2>

{likedGames.length > 0 && (
<div className="mb-6">
<input
type="text"
value={searchLikedGames}
onChange={e => setSearchLikedGames(e.target.value)}
placeholder="Search liked games..."
className="w-full px-4 py-2 border border-foreground/20 rounded-md font-mono text-base bg-background focus:outline-none focus:border-red-500"
/>
</div>
)}
...
{loadingLikedGames ? (
<p className="text-foreground/60">Loading liked games...</p>
) : likedGames.length === 0 ? (
<p className="text-foreground/60 font-mono text-sm text-center pt-5 pb-10">You haven't liked any games yet. <Link href={'/marketplace'} className={'text-red-500 hover:underline hover:text-red-700'}>Browse</Link> the marketplace!</p>
) : filteredLikedGames.length === 0 ? (
<p className="text-foreground/60 font-mono text-sm text-center pt-5 pb-10">No liked games found matching your search.</p>
) : (
<div className="space-y-3">
{filteredLikedGames.map(game => (
<div
key={game.game_id}
className="flex justify-between items-center p-4 bg-foreground/5 rounded border border-foreground/10"
>
...
<h3 className="font-medium">{game.title}</h3>
{game.author && (
<p className="text-xs text-foreground/50 font-mono">by {game.author}</p>
)}
{game.description && (
<p className="text-sm text-foreground/60 mt-1">{game.description}</p>
)}
